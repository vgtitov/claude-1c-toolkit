...
3. **Центральный MCP по HTTP:** ✅ обкатан на боевом наборе (`ONEC_SRC_DIR=C:\dev\tvg\rp`, 11 слоёв) — SSE-транспорт, discovery/search/find_object отдаются по HTTP корректно (клиент `tests/smoke_http_client.py`). Осталось: прогнать через сам docker-compose (демон Docker Desktop был выключен), добавить простую auth/обратный прокси перед выставлением за пределы localhost.
