...
## Переключение клиента local ↔ central (`scripts/switch_erp.py`)
Чтобы не править `.mcp.json` руками, переключение источника erp-1c автоматизировано — скрипт переписывает ТОЛЬКО
блок `erp-1c`, остальные серверы не трогает:
```bash
uv run scripts/switch_erp.py --mode auto      # auto|local|central; по умолчанию auto
```
- **local** — stdio-движок читает клоны из `ONEC_SRC_DIR` (блок берётся из `mcp/<profile>.mcp.json`).
- **central** — SSE к центру; URL/токен — env-плейсхолдеры `${ERP1C_URL}`/`${ERP1C_TOKEN}` (секрет в файл НЕ пишется).
- **auto** — если `ERP1C_URL` задан и центр доступен по TCP → central, иначе local (безопасный откат).

Сценарий безболезненного перехода: команда стартует на `local` (центр ещё не поднят). Когда центр готов — раздать
`ERP1C_URL` и `ERP1C_TOKEN` (env) и выполнить `switch_erp.py --mode auto` (его может запустить и сам ассистент),
...
Чтобы разработчику не возиться с `setx`/`export` руками, в паре к `switch_erp.py` идёт **`scripts/set_token.ps1` /
`scripts/set_token.sh`**: спрашивает токен скрыто, кладёт `ERP1C_URL`/`ERP1C_TOKEN`/`ERP1C_MODE=auto` в окружение
пользователя и печатает готовый блок-инструкцию, который разработчик просто вставляет в Claude Code — дальше
ассистент сам проходит onboard → `switch_erp --mode central` → self-test.
...
Принципы (те же, что у `switch_erp`): секрет — **только в env пользователя**, никогда в файл/git/чат; в `.mcp.json`
попадает лишь плейсхолдер `${ERP1C_TOKEN}`. Скрипт идемпотентен и не оставляет токен на экране (ввод скрыт).
