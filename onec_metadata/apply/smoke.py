...
_HANG_HINT = (
    ...
    "Тогда запусти команду при человеке и посмотри, не появилось ли окно на экране. "
    "Если окна НЕТ, а файл-результат так и не создан — сценарий не начинал исполняться, "
    "то есть сеанс не дошёл до открытия обработки. Локальные данные на этом кончаются: "
    "причину видно только со стороны сервера — журнал регистрации базы и список сеансов "
    "кластера (rac session list). Это к тем, у кого есть доступ к кластеру.")
