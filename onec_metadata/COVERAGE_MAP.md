...
- **`set-synonym`** — синоним/«Наименование» существующего языка (объект или `--member`), оба формата, свободный
  текст, byte-perfect. Безопасно (презентация, ссылок не ломает). Добавление нового языка — отдельная операция.
...
- **Составной тип** (несколько типов в одном поле) ✅ 2026-07-15 — `type_ref` списком (CLI: повторить `--type`);
  квалификаторы к составному не применяются, из составного в одиночный сворачивается. Оба формата.
