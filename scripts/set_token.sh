#!/usr/bin/env bash
# Подключение к ЦЕНТРАЛЬНОМУ onec-code одним шагом (org-agnostic, mac/Linux): кладёт URL и bearer-токен центра
...
# не пишется кроме env пользователя: не в git, не в .mcp.json. switch_source подставит его плейсхолдером
# ${ONEC_MCP_TOKEN} только при старте MCP. Локализации вендорят скрипт и задают свои URL/профиль.
#
# Запуск:
...
#   ONEC_MCP_TOKEN=... bash scripts/set_token.sh --url http://host:8000/sse # токен из env (не печатается)
set -euo pipefail
URL="${ONEC_MCP_URL:-}"; PROFILE="dev"; TOKEN="${ONEC_MCP_TOKEN:-}"; NOTEST=0
...
touch "$RC"
grep -v -E '^export ONEC_MCP_(URL|TOKEN|MODE)=' "$RC" > "$RC.tmp" && mv "$RC.tmp" "$RC"
{ echo "export ONEC_MCP_URL=\"$URL\""; echo "export ONEC_MCP_TOKEN=\"$TOKEN\""; echo "export ONEC_MCP_MODE=\"auto\""; } >> "$RC"
export ONEC_MCP_URL="$URL"; export ONEC_MCP_TOKEN="$TOKEN"; export ONEC_MCP_MODE="auto"
ok "ONEC_MCP_URL = $URL (свой центр — перезапусти с --url <адрес>)"; ok "ONEC_MCP_TOKEN = <скрыт, длина ${#TOKEN}>"; ok "ONEC_MCP_MODE = auto"
