...
`scripts/zabbix_perf.py`. Правило двух тактов: снять данные ПОЛНЫМ охватом (и дашборд, И все хосты контура,
окно 24ч) → синтезировать отчёт (коррелировать находки в «болезни», интерпретировать через инвентарь хостов,
каждой рекомендации — исполнитель и ожидаемый эффект, приоритет по эффекту, а не по severity). Пересказ
сырого вывода инструмента отчётом НЕ является. Методика, формат отчёта, анти-паттерны —
`references/zabbix-perf-analysis.md`.
