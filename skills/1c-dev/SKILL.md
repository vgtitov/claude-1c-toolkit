...
Алгоритм поиска (MCP-first): `find_object` → `search` → `read_module` → grep по клонам. **Не делай вывод об
отсутствии по непросмотренному слою.**
...
**не-код (метаданные/формы/СКД) создаёт человек в IDE; код пишет и проверяет AI.**
